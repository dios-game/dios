...
#include "dios_util_boost/util_boost_buffer.h"
#include "dios_util_boost/util_boost_serializer.h"
...
namespace Foundation{
	...
	namespace NetSession{
		...
		public:
			...
			// 会话服务监听;
			virtual ISessionServer::Ptr SessionListen( const char * local_ip, int local_port, const ComponentID& session_component_id, const ComponentID& packer_component_id="dios_packer" ) = 0;
			// 会话连接(指定内部创建会话对象);
			virtual ISession::Ptr SessionConnect( const char * remote_ip, int remote_port, const ComponentID& session_component_id, const ComponentID& packer_component_id="dios_packer", Foundation::TaskService::IStrandTask::Ptr rector_strand = Foundation::TaskService::IStrandTask::Ptr() ) = 0;
			...
			// 会话连接(指定会话对象);
			virtual bool SessionConnect( const char * remote_ip, int remote_port, const ISession::Ptr& session, const ComponentID& packer_component_id="dios_packer", Foundation::TaskService::IStrandTask::Ptr rector_strand = Foundation::TaskService::IStrandTask::Ptr() ) = 0;
