...
# 整理标准源码目录
# 
MACRO( _dios_builder_module_add_sources )
	...
	if(DIOS_CMAKE_PLATFORM_WIN32 OR DIOS_CMAKE_PLATFORM_WIN64)
		...
		_internal_src_import_from_dir( resources .lua .xml .json)
		_internal_src_import_from_dir( build .bat )
	...
	elseif(DIOS_CMAKE_PLATFORM_IOS)
		_internal_src_import_from_dir( src.ios/cpp .h .hpp .hxx .c .cpp .cxx .cc .m .mm .pch )
	elseif(DIOS_CMAKE_PLATFORM_MAC)
		_internal_src_import_from_dir( src.mac/cpp .h .hpp .hxx .c .cpp .cxx .cc .m .mm .pch )
...
MACRO( _internal_src_import_from_dir SUB_DIR )
	...
	# 组合绝对路径
	SET(FULL_DIR ${DIOS_MODULE_${DIOS_BUILDER_MODULE}_DIRECTORY}/${SUB_DIR})		
	# 检测本路径是否为文件夹
	if( (EXISTS ${FULL_DIR}) AND (IS_DIRECTORY ${FULL_DIR}) )
		
		# 增加源码路径列表
		LIST(APPEND DIOS_SRC_SUB_DIRECTORIES ${SUB_DIR})
		...
		FOREACH(TEMP_FILE_EXTENSION ${ARGN})
			...
			FOREACH( TEMP_FILE ${TEMP_FILES} )				
				# 增加源码到路径源码列表
				LIST(APPEND DIOS_SRC_SUB_DIRECTORY_FILES_${SUB_DIR} ${TEMP_FILE})
				LIST(APPEND DIOS_MODULE_${DIOS_BUILDER_MODULE}_SOURCES ${TEMP_FILE})
			ENDFOREACH()
		ENDFOREACH()
...
MACRO( _internal_source_clear )
	...
	FOREACH(TEMP_SRC_DIRECTORY ${DIOS_SRC_SUB_DIRECTORIES})
		UNSET(DIOS_SRC_SUB_DIRECTORY_FILES_${TEMP_SRC_DIRECTORY})
	ENDFOREACH()
	UNSET(DIOS_SRC_SUB_DIRECTORIES)
	UNSET(DIOS_MODULE_${DIOS_BUILDER_MODULE}_SOURCES)
ENDMACRO()
...
MACRO( _internal_source_to_group )
	...
	FOREACH(TEMP_SRC_DIRECTORY ${DIOS_SRC_SUB_DIRECTORIES})
		dios_module_add_includes(${DIOS_BUILDER_MODULE} ${DIOS_MODULE_${DIOS_BUILDER_MODULE}_DIRECTORY}/${TEMP_SRC_DIRECTORY})
