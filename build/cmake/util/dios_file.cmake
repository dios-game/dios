...
#     文件拷文件
# 
function(dios_file_cp SOURCE DEST)
			...
			if( (EXISTS ${DEST_TEMP}) AND (IS_DIRECTORY ${DEST_TEMP}))
				...
				if(${DEST_TEMP} MATCHES ".*/$")
					...
					if( (NOT DEST_NAME) OR (NOT ${SOURCE_NAME} STREQUAL ${DEST_NAME}) )
						dios_file_mv( ${DEST_DIR}/${SOURCE_NAME} ${DEST_TEMP})
					endif()
				endif()
			endif()
		else()
			...
			foreach( ONE_FILE ${FILES})
				...
				if( DEST_TEMP )
					if(IS_DIRECTORY ${ONE_FILE})
						if(${DEST_TEMP} MATCHES ".*/$")
							dios_file_cp( ${ONE_FILE}  ${DEST_TEMP})
						else()
							dios_file_cp( ${ONE_FILE}  ${DEST_TEMP}/)
						endif()
					else()
						if(${DEST_TEMP} MATCHES ".*/$")
							dios_file_cp( ${ONE_FILE}  ${DEST_TEMP}${ONE_FILE_NAME})
						else()
							dios_file_cp( ${ONE_FILE}  ${DEST_TEMP}/${ONE_FILE_NAME})
						endif()
					endif()
...
function(dios_file_copy SOURCE DEST)
	dios_file_cp(${SOURCE} ${DEST})
endfunction()
#
# mv 移动
# 
function(dios_file_mv OLD_FILE NEW_FILE)
...
# mvdir 移动文件夹
#
function(dios_file_mvdir OLD_DIR NEW_DIR)
	file(RENAME ${OLD} ${NEW})
endfunction()

# 
# dios_file_md5file 计算file的md5
#
function(dios_file_md5file FILE_NAME MD5_VALUE)
...
# 
# dios_file_md5dir 计算dir的md5
#
function(dios_file_md5dir DIR_NAME MD5_VALUE)
	...
	if( (EXISTS ${FULL_DIR}) AND (IS_DIRECTORY ${FULL_DIR}) )
		...
		foreach(TEMP_FILE ${TEMP_FILES})
			if(IS_DIRECTORY ${TEMP_FILE})
				...
				dios_file_md5dir(${TEMP_FILE} ${MD5_VALUE})
			else()
				dios_file_md5file(${TEMP_FILE} ${MD5_VALUE})
			endif()
		endforeach()
	endif()
...
function(internal_tar_win32)
	...
	set(TAR_PROGRAM ${DIOS_CMAKE_TOOL_DIRECTORY}/win/7za.exe)
...
# my_bash：执行批处理命令
#
function(dios_file_bash NAME WORK_DIR)
	if(WIN32 OR ANDROID)
