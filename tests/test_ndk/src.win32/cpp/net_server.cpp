...
{
	CServer * server = (CServer*)ptr;
	// XE_COMPONENT_LOG(// XE_COMPONENT_LOG_LEVEL_ERROR, "OnAcceptError Appear.");
//	server->ShutdownReal();
}
...
bool CServer::Init( const char * ip, unsigned int port, IConnectorSink* sink ) {
	...
	if(evconn_listener_ == 0) {
		...
		// XE_COMPONENT_LOG(// XE_COMPONENT_LOG_LEVEL_ERROR, "server(%s:%d) evconnlistener_new_bind failed.", ip, port);
		return false;
	}
	...
	local_ip_ = ip;
	local_port_ = port;
	// XE_COMPONENT_LOG(// XE_COMPONENT_LOG_LEVEL_INFO, "server(%s:%d) start", local_ip_.c_str(), local_port_);
	return true;
}
...
void CServer::ShutdownReal()
{
	...
	// XE_NDC("CServer::ShutdownReal");
	// XE_COMPONENT_LOG(// XE_COMPONENT_LOG_LEVEL_INFO, "server(%s:%d) shutdown", local_ip_.c_str(), local_port_);
