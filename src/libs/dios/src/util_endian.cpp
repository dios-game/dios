...
void CEndianHelper::FlipEndian( void* data, int size )
{
	...
	{
		swapByte = *(char*)((dios_intptr)data + byteIndex);
		*(char*)((dios_intptr)data + byteIndex) = *(char*)((dios_intptr)data + size - byteIndex - 1);
		*(char*)((dios_intptr)data + size - byteIndex - 1) = swapByte;
	}
}
