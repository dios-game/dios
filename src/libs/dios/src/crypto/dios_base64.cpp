...
void
dios_base64_encode_init (struct dios_base64_state *state)
{
	state->eof = 0;
	state->bytes = 0;
	state->carry = 0;
...
void
dios_base64_encode_update (struct dios_base64_state *state, const char *const src, dios_uint32 srclen, char *const out, dios_uint32 *const outlen)
{
...
void
dios_base64_encode_final (struct dios_base64_state *state, char *const out, dios_uint32 *const outlen)
{
	char *o = out;
...
void
dios_base64_decode_init (struct dios_base64_state *state)
{
	state->eof = 0;
	state->bytes = 0;
	state->carry = 0;
...
int
dios_base64_decode_update (struct dios_base64_state *state, const char *const src, dios_uint32 srclen, char *const out, dios_uint32 *const outlen)
{
	int ret = 0;
	const char *c = src;
	char *o = out;
...
void
dios_base64_encode (const char *const src, dios_uint32 srclen, char *const out, dios_uint32 *const outlen)
{
	dios_uint32 s;
	dios_uint32 t;
	struct dios_base64_state state;

	/* Init the stream reader: */
	dios_base64_encode_init(&state);

	/* Feed the whole string to the stream reader: */
	dios_base64_encode_update(&state, src, srclen, out, &s);

	/* Finalize the stream by writing trailer if any: */
	dios_base64_encode_final(&state, out + s, &t);
...
int
dios_base64_decode (const char *const src, dios_uint32 srclen, char *const out, dios_uint32 *outlen)
{
	struct dios_base64_state state;

	dios_base64_decode_init(&state);
	return dios_base64_decode_update(&state, src, srclen, out, outlen);
}

void dios_base64_decode_final( struct dios_base64_state *, char *const out, dios_uint32 *outlen )
{
	*outlen = 0;
}
