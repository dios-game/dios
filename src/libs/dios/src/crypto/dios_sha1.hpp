#ifndef __DIOS_SHA1_H__
#define __DIOS_SHA1_H__
...
public:
	static void Calculate(const void *lpBuffer, size_t cbSize, Digest &digest)
	{
		...
		sha1._appendData(lpBuffer, cbSize);
		sha1._getDigest(digest);
	}

	static std::string StringCalculate(const void *lpBuffer, size_t cbSize)
	{
		SHA1::Digest digest;
		SHA1::Calculate(lpBuffer, cbSize, digest);

		return _SHA1DigestToString(digest);
	}

	DIOS_SINGLETON(SHA1);

	void CalculateX(const void *lpBuffer, size_t cbSize, Digest &digest)
	{
		_appendData(lpBuffer, cbSize);
		_getDigest(digest);
	}

	std::string StringCalculateX(const void *lpBuffer, size_t cbSize)
	{
		SHA1::Digest digest;
		Calculate(lpBuffer, cbSize, digest);

		return _SHA1DigestToString(digest);
	}
private:

	static std::string _SHA1DigestToString(SHA1::Digest digest)
	{
		char szDigest[(SHA1::DIGEST_LENGTH * 4 + 1) * 2] = {};
	...
	void _appendData(const void *lpBuffer, size_t cbSize)
	{
		size_t cbCopied = 0;
...
private:
	...
	static inline unsigned int BitSwap(unsigned int x)
	{
		...
		unsigned char buff_rlt[4];
		*(int*) buff = x;
		buff_rlt[3] =  buff[0];
		buff_rlt[2] =  buff[1];
		buff_rlt[1] =  buff[2];
		buff_rlt[0] =  buff[3];
		return *(unsigned int*)buff_rlt;
#endif // DIOS_TARGET_PLATFORM == DIOS_PLATFORM_WIN32
	}
};

#endif // __DIOS_SHA1_H__
