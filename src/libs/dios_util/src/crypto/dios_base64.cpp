...
void
ds_base64_encode_init (struct ds_base64_state *state)
{
	state->eof = 0;
	state->bytes = 0;
	state->carry = 0;
...
void
ds_base64_encode_update (struct ds_base64_state *state, const char *const src, ds_uint32 srclen, char *const out, ds_uint32 *const outlen)
{
...
void
ds_base64_encode_final (struct ds_base64_state *state, char *const out, ds_uint32 *const outlen)
{
	char *o = out;
...
void
ds_base64_decode_init (struct ds_base64_state *state)
{
	state->eof = 0;
	state->bytes = 0;
	state->carry = 0;
...
int
ds_base64_decode_update (struct ds_base64_state *state, const char *const src, ds_uint32 srclen, char *const out, ds_uint32 *const outlen)
{
	int ret = 0;
	const char *c = src;
	char *o = out;
...
void
ds_base64_encode (const char *const src, ds_uint32 srclen, char *const out, ds_uint32 *const outlen)
{
	ds_uint32 s;
	ds_uint32 t;
	struct ds_base64_state state;

	/* Init the stream reader: */
	ds_base64_encode_init(&state);

	/* Feed the whole string to the stream reader: */
	ds_base64_encode_update(&state, src, srclen, out, &s);

	/* Finalize the stream by writing trailer if any: */
	ds_base64_encode_final(&state, out + s, &t);
...
int
ds_base64_decode (const char *const src, ds_uint32 srclen, char *const out, ds_uint32 *outlen)
{
	struct ds_base64_state state;

	ds_base64_decode_init(&state);
	return ds_base64_decode_update(&state, src, srclen, out, outlen);
}

void ds_base64_decode_final( struct ds_base64_state *, char *const out, ds_uint32 *outlen )
{
	*outlen = 0;
}
