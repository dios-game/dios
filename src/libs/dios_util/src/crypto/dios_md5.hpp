...
public:
	static void Calculate(const void *lpBuffer, size_t cbSize, Digest &digest)
	{
		...
		md5._appendData(lpBuffer, cbSize);
	...
	{
		MD5::Digest digest;
		MD5::Calculate(lpBuffer, cbSize, digest);

		return _MD5DigestToString(digest);
	}
	...
	{
		MD5::Digest digest;
		CalculateX(lpBuffer, cbSize, digest);

		return _MD5DigestToString(digest);
	}
private:

	static std::string _MD5DigestToString(MD5::Digest digest)
	{
		char szDigest[(MD5::DIGEST_LENGTH + 1) * 2] = {};

		for (size_t i = 0; i < MD5::DIGEST_LENGTH; ++i)
		{
