#ifndef __DS_UTIL_EVENT_SET_HPP___
#define __DS_UTIL_EVENT_SET_HPP___
...
public:
	struct MapItem{
		MapItem(ds_uint32 a_key, std::function<Signature> a_object) { key=a_key; object=a_object; next_ = 0;}
		ds_uint32 key;
		MapItem* next_;
	...
	std::function<Signature>* find(ds_uint32 key){
	...
	ds_uint32 add(const Function& obj){
		
		if(alloc_id_ == MAX_EVENT_ALLOC_ID){
			alloc_id_ = 0;
		}
		ds_uint32 id = ++ alloc_id_;
	...
	ds_boolean remove(ds_uint32 key){
		MapItem* item = head_;
		MapItem* pre_item = 0;
		while(item){
			if(item->key == key){
	...
	template<class Function>
	ds_boolean remove(const Function& object){
		MapItem* item = head_;
		MapItem* pre_item = 0;
		while(item){
	...
	std::map<ds_uint32, std::function<Signature> > get_map(void){
		std::map<ds_uint32, std::function<Signature> > map;
		MapItem* item = head_;
		while(item){
...
class DS_UTIL_DECL _IEvent: public std::enable_shared_from_this<_IEvent>
{
public:
	virtual void Disconnect(ds_uint32 id) = 0;
};
...
class DS_UTIL_DECL CEventListener
{
public:
	CEventListener(void) {
		id_ = 0;
	}
	CEventListener(ds_uint32 id, std::weak_ptr<_IEvent> event) {
		id_ = id;
		event_ = event;
	}
	~CEventListener(void) {}
...
class DS_UTIL_DECL CDummyMutex
{
public:
	void lock(void) {}
...
public:
	...
	CEventListener Connect(const Function& callback) { 
		...
		ds_uint32 id = _Connect(callback);
		if(id != 0) {
	...
	void operator()(void) { 
		...
		std::map<ds_uint32, std::function<Signature> > callback_map = _GetCallbackMap();
...
protected:
	...
	// 插入监听;
	template<class Function>
	ds_uint32 _Connect(const Function& callback) {
	...
	// 获取监听表单;
	std::map<ds_uint32, std::function<Signature> > _GetCallbackMap(void) {
