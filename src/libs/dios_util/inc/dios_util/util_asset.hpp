#ifndef __DS_UTIL_ASSET_HPP__
#define __DS_UTIL_ASSET_HPP__
...
class DS_UTIL_DECL CAssetLoader{
public:
	DS_SINGLETON(CAssetLoader);
	DS_REF_PROPERTY(std::function<void*(const char*, ds_boolean, unsigned long*)>, read_file_function);
	DS_REF_PROPERTY(std::function<void(void*)>, free_file_data_function);
	DS_REF_PROPERTY(std::function<std::string(const std::string&)>, full_path_for_file_name_function);

	void* ReadFile(const char* file_name, ds_boolean is_async, unsigned long* size){
...
class DS_UTIL_DECL CAssetLocale{
public:
	DS_SINGLETON(CAssetLocale);
	DS_REF_PROPERTY(std::function<std::string(ds_int32)>, locale_get_string);
private:
	CAssetLocale() {}
};
...
public:
	...
	T Get(const std::string& key, T default_value) const {
		...
		return dios::util::lexical_cast<T>(temp_node->value());
	}
	...
	std::string Get(const std::string& key, std::string default_value) const {
		...
		if(CAssetLocale::GetInstancePtr()->locale_get_string()){
			...
			if(temp_node && temp_node->value_size() > 0){
				ds_int32 string_id = dios::util::lexical_cast<ds_int32>(temp_node->value());;			
		...
		return dios::util::lexical_cast<std::string>(temp_node->value());
	}
	...
	T GetValue(T default_value) const {
		...
		return dios::util::lexical_cast<T>(node_->value());
	}
...
public:
	...
	// 从文件中读取;
	bool LoadFile(const std::string& file_name, ds_boolean is_async = false) {
	...
	// 从内存中读取;
	bool LoadData(const void* data, ds_uint32 size) {
