...
bool CNetService::Initialize( Foundation::LogService::ILog::Ptr log, unsigned int recv_size ) {
	...
	DIOS_COMPONENT_LOG.set_log(log);
	DIOS_NDC("NetService::Initialize");

	boost::unique_lock<boost::shared_mutex> lock(running_mutex_);
	if(is_running_) 
	{
		DIOS_COMPONENT_LOG( DIOS_COMPONENT_LOG_LEVEL_ERROR, "net service is initialized");
		return false;
	}

	if(recv_size == 0) {
		DIOS_COMPONENT_LOG(DIOS_COMPONENT_LOG_LEVEL_ERROR, "recv_size cannot be 0" );
		return false;
	}
	...
	if(WSAStartup(MAKEWORD(2,2), &wsaData) != 0) {
		DIOS_COMPONENT_LOG( DIOS_COMPONENT_LOG_LEVEL_ERROR, "net service initialize WSAStartup failed.");
		return false;
	}

	DIOS_COMPONENT_LOG( DIOS_COMPONENT_LOG_LEVEL_TRACE, "wsastartup ok.");

	//	about thread and lock
	evthread_use_windows_threads();
	DIOS_COMPONENT_LOG( DIOS_COMPONENT_LOG_LEVEL_TRACE, "evthread_set ok.");

	//	get count of processors
	SYSTEM_INFO sysInfo;
	GetSystemInfo(&sysInfo);
	DIOS_COMPONENT_LOG( DIOS_COMPONENT_LOG_LEVEL_TRACE, "getsysteminfo ok. numberofprocessors: %u.", sysInfo.dwNumberOfProcessors);
	...
	DIOS_COMPONENT_LOG( DIOS_COMPONENT_LOG_LEVEL_TRACE, "event_config_set ok.");
#else
	//	about thread and lock
	...
	DIOS_COMPONENT_LOG( DIOS_COMPONENT_LOG_LEVEL_TRACE, "event_base_new ok.");
	...
	DIOS_COMPONENT_LOG( DIOS_COMPONENT_LOG_LEVEL_TRACE, "ev_token_bucket_cfg_new ok.");
	...
	event_add(ev_, &tv);
	DIOS_COMPONENT_LOG( DIOS_COMPONENT_LOG_LEVEL_TRACE, "event_add timer ok.");
	
	// run loop callback
	pthread_create(&loop_thread_, 0, loopThreadStartFunc, this);
	DIOS_COMPONENT_LOG( DIOS_COMPONENT_LOG_LEVEL_TRACE, "main thread create ok.");

	//	start
	is_running_ = true;
	DIOS_COMPONENT_LOG( DIOS_COMPONENT_LOG_LEVEL_INFO, "Initialize Finish.");
	return true;
}

bool CNetService::Shutdown( void ) {
	
	DIOS_NDC("NetService::Shutdown");
	...
	if(!is_running_) 
	{
		DIOS_COMPONENT_LOG( DIOS_COMPONENT_LOG_LEVEL_ERROR, "net service is shutdowned.");
		return false;
	}
	is_running_ = false;
	...
	net_shutdown_event_();
	DIOS_COMPONENT_LOG( DIOS_COMPONENT_LOG_LEVEL_TRACE, "shutdown_signal_.num_slots: %d.", net_shutdown_event_.NumListeners());
	...
	DIOS_COMPONENT_LOG( DIOS_COMPONENT_LOG_LEVEL_TRACE, "waiting for thread exit.");
	...
	DIOS_COMPONENT_LOG( DIOS_COMPONENT_LOG_LEVEL_TRACE, "free event environment ok.");
	DIOS_COMPONENT_LOG( DIOS_COMPONENT_LOG_LEVEL_INFO, "Shutdown Finish.");
	return true;
}

IServer::Ptr CNetService::Listen( const char * local_ip, int local_port, IConnectorSink* sink ) {
	
	DIOS_NDC("NetService::Listen");
...
IConnector::Ptr	CNetService::Connect( const char * remote_ip, int remote_port, IConnectorSink* sink ){
	...
	if(connect_fd <= 0) {
		DIOS_COMPONENT_LOG( DIOS_COMPONENT_LOG_LEVEL_ERROR, "request socket failed.(connector(%s:%d)", remote_ip, remote_port);
		return IConnector::Ptr();
	}
	...
	if(result == -1) {
		DIOS_COMPONENT_LOG( DIOS_COMPONENT_LOG_LEVEL_ERROR, "connect failed. error code: %d.(connector(%s:%d)", result, remote_ip, remote_port);
		return IConnector::Ptr();
	}
...
{
	DIOS_NDC("NetService::NullConnect");
	int connect_fd = socket(PF_INET, SOCK_STREAM, 0);
	if(connect_fd <= 0) {
		DIOS_COMPONENT_LOG( DIOS_COMPONENT_LOG_LEVEL_ERROR, "request socket failed.(connector(%s:%d)", remote_ip, remote_port);
		return;
