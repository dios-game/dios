...
void onRead( struct bufferevent * bev, void * ctx ) {
	...
	do {
		...
		{	
			...
			sLogTrace("read data(%d) [connector(%s:%d)]", read_size, connector->remote_ip().c_str(), connector->remote_port());
		}
...
bool CConnector::Init( int sock_id, IConnectorSink* sink, struct sockaddr* remote_addr ) {
	...
	sock_id_ = sock_id;
	bufferev_ = std::shared_ptr<bufferevent>(bufferevent_socket_new(net_service_impl_->ev_base(), sock_id, BEV_OPT_THREADSAFE), [](bufferevent* p){ bufferevent_free(p); });
	if(!bufferev_) {
		net_shutdown_listener_.Disconnect();
		sLogError("create bufferevent socket failed.");
		return false;
	}
...
//	bufferevent_set_rate_limit(bufferev_.get(), net_service_impl_->ev_t_bucket_cfg());
	...
	sLogTrace("init connector ok.[connector(%s:%d)]", remote_ip_.c_str(), remote_port_);
	return true;
}

bool CConnector::Send( const void * buffer, unsigned int size ) {
	
	sLogTrace("[%ud] send data(%d)[connector(%s:%d)]", dios::util::CTimer::GetMilliSecond(), size, remote_ip_.c_str(), remote_port_);
	if(shutdown_) {
		sLogError("connector is shutdowned yet[connector(%s:%d)]", remote_ip_.c_str(), remote_port_);
		return false;
	}

	auto bufferev = bufferev_weak_.lock();
	if(bufferev){
		bufferevent_write(bufferev.get(), buffer, size);
		return true;
	}

	sLogError("bufferev is null[connector(%s:%d)]", remote_ip_.c_str(), remote_port_);
	return false;
}

void CConnector::Shutdown( void ) {

	sLogTrace("shutdown connector[connector(%s:%d)]", remote_ip_.c_str(), remote_port_);
	if(shutdown_) {
		sLogError("connector is shutdowned yet[connector(%s:%d)]", remote_ip_.c_str(), remote_port_);
		return;
	}
...
void CConnector::ShutDownImmediately( void ) {
	...
	sLogTrace("shutdown connector immediately[connector(%s:%d)]", remote_ip_.c_str(), remote_port_);
	shutdown_ = true;
	if(bufferev_)
	{
