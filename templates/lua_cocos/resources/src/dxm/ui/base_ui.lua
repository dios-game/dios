...
function CBaseUI:set_widget(widget)
    ...
    if self.widget == widget or self.is_entered then
        return
    end
...
function CBaseUI:SetPosition(x, y)
    ...
    if self.is_entered ==true then
        self:Root():setPosition(cc.p(self.position.x, self.position.y))
    end
end
...
function CBaseUI:_Enter()
    ...
    self:PlayAnimation("OpenAnimation", false)
end
...
-- 设置可触摸性
function CBaseUI:SetWidgetTouchEnable(is_enable, ...)
    self:SetWidgetTouchEnableFromWidget(is_enable, self.widget, ...)
end

function CBaseUI:SetWidgetTouchEnableFromWidget(is_enable, widget, ...)
    local widget = self:GetWidgetFromWidget(widget, ...)
    if widget ~= nil then
        widget:setTouchEnabled(is_enable)
    end
end
...
-- 播放动画
function CBaseUI:PlayAnimation(animation_name, loop)
    if self.action then
